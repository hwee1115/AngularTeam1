angular.module("app")
    ...
    $scope.getList = (pageNo, review_score, searchType, searchContent) => {
    ReviewsService.list(pageNo, review_score, searchType, searchContent)
        .then((response) => {
    ...
    $scope.ScoreConfirm = (review_score) => {
      $scope.reviewScoreVal = review_score;
      $scope.getList(1,$scope.reviewScoreVal,null,null);
      };
    $scope.SearchConfirm = (searchType, searchContent) => {
      $scope.searchTypeVal = searchType;
      $scope.searchContentVal = searchContent;
      $scope.getList(1,0,$scope.searchTypeVal,$scope.searchContentVal);
      };
    ...
    //           $scope.pager = response.data.pager;
    //           $scope.reviewPageVal = review_scoreVal;
    //           $scope.reviews = response.data.reviews;
    //           $scope.review_scoreVal = review_scoreVal; // *** 
    //           $scope.pageRange = [];
    //           for(var i=$scope.pager.startPageNo; i<=$scope.pager.endPageNo; i++){
    //               $scope.pageRange.push(i)
    //           }
